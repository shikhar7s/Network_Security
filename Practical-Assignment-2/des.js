...
const des = (msg, key, subkeys) => {
  ...
  return {
      cipherText: chunkString(enc, 4).map(binToHex).join(""),
      data,
  }
}
...
const encodeHex = (msg, key) => {
    const binMsg = bin(msg.toUpperCase());
    const binKey = bin(key.toUpperCase());
    return encode(binMsg, binKey);
}
